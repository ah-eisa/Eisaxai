"""
api/routes/portfolio.py — Extracted portfolio management routes

Extracted from api_bridge_v2.py (Phase 2 — Step 2 of 3).
All endpoints use Depends(require_auth) — no raw token checks.

Endpoints extracted:
  POST   /v1/upload-portfolio                  (line 298)
  GET    /v1/portfolio/history/{user_id}        (line 1650)
  GET    /v1/portfolio/snapshot/{snapshot_id}   (line 1679)
  GET    /v1/portfolio/compare                  (line 1705)
  POST   /v1/global-allocate                    (line 1762)
  GET    /v1/global-allocate/profiles           (line 1813)
  GET    /v1/portfolio/performance/{user_id}    (line 1840)
"""
...
# ── Shared helpers (from api_bridge_v2.py top-level) ─────────────────────
...
@router.post("/v1/upload-portfolio")
@_limit("10/minute")
...
# ══════════════════════════════════════════════════════════════════════════════
def _is_portfolio_admin(user: dict) -> bool:
    return user.get("role") == "admin" or str(user.get("user_id")) == "admin"


def _require_portfolio_owner(user: dict, owner_user_id: str) -> None:
    if _is_portfolio_admin(user):
        return
    if str(owner_user_id) != str(user.get("user_id", "")):
        raise HTTPException(status_code=403, detail="Forbidden")


@router.get("/v1/portfolio/history/{user_id}")
@_limit("30/minute")
async def portfolio_history(
    ...
    try:
        ...
        _require_portfolio_owner(user, user_id)
...
@router.get("/v1/portfolio/snapshot/{snapshot_id}")
@_limit("30/minute")
async def get_portfolio_snapshot(
    ...
    try:
        ...
        _require_portfolio_owner(user, snap.get("user_id", ""))
        return snap
...
@router.get("/v1/portfolio/compare")
@_limit("20/minute")
async def compare_portfolio_snapshots(
    ...
    try:
        ...
        _require_portfolio_owner(user, snapshot_a.get("user_id", ""))
        _require_portfolio_owner(user, snapshot_b.get("user_id", ""))
...
@router.get("/v1/global-allocate/profiles")
@_limit("60/minute")
...
@router.get("/v1/portfolio/performance/{user_id}")
@_limit("30/minute")
